...
# Parser for SayPlus1; hackily copied from the Rails parser
# TODO: cleanly extend the RailsParser
class Sayplus1Parser < Parser
  def parse( line )
    ...
    if url
      ...
      # Events to pop up
      add_event(:block => 'info', :name => "OAuths", :message => "OAuth", :update_stats => true, :color => [0.5, 1.0, 0.5, 1.0]) if url.include?('oauth=success')
      add_event(:block => 'info', :name => "Follow", :message => "Follow", :update_stats => true, :color => [0.5, 1.0, 0.5, 1.0]) if url.include?('new_follow')
